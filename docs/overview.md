...
SearXNG Search Engine MCP Server for Agentic AI!
...
```json
{
  "mcpServers": {
    "searxng-mcp": {
      "command": "uv",
      "args": ["run", "--with", "searxng-mcp", "searxng-mcp"],
      "env": {}
    }
  }
}
...
```bash
searxng-mcp --transport streamable-http --port 8001
```
