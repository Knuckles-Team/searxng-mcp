...
def create_agent_server(
    ...
    # Create A2A app with proper lifespan handling
...
def agent_server():
    ...
    create_agent_server(
        ...
        ssl_verify=not args.insecure,  # If insecure flag is present, ssl_verify is False
    )


def usage():
    print(
        f"Searxng Mcp ({__version__}): CLI Tool\n\n"
        "Usage:\n"
        ...
        "\n"
        "Examples:\n"
        "  [Simple]  searxng-agent \n"
        '  [Complex] searxng-agent --host "value" --port "value" --debug "value" --reload --provider "value" --model-id "value" --base-url "value" --api-key "value" --mcp-url "value" --mcp-config "value" --skills-directory "value" --web\n'
    )
